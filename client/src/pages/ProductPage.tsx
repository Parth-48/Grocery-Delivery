...
const ProductPage = () => {
  ...
  useEffect(() => {
    ...
    }).catch(()=>navigate("/prooducts")).finally(()=>setLoading(false))
  ...
  return (
    <div className="min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="bg-white/50 rouned-2xl overflow-hidden">
          <div className="grid md:grid-cols-2 gap-0">
            ...
            <div className="relative flex-center p-8 md:p-12 min-h-[320px] md:min-h-[480px]">
              ...
              <div className="absolute top-5 left-5 flex-wrap gap-1.5">
              {product.isOrganic && (
                <span className="flex mb-1 items-center gap-1 px-2.5 py-1 text-xs font-semibold bg-app-green text-white rounded-full">
              ...
              {product.rating>0 && (
                <div className="flex items-center gap-2 mb-5">
                  <div className="flex items-center gap-0.5">
                    {[1, 2, 3, 4 , 5].map((star) => <StarIcon key={star} className={`w-4 h-4  ${star <= Math.round(product.rating) ? "text-app-warning fill-app-warning" : "text-app-border"}`}/>)}
                  </div>
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center border border-app-border rounded-xl overflow-hidden">
                  <button className="p-3 hover:bg-app-cream transition-colors">
                    <MinusIcon onClick={handleMinus} className="w-4 h-4"/>
                  </button>
                  <span>{displayQuantity}</span>
                  <button className="p-3 hover:bg-app-cream transition-colors">
                    <PlusIcon onClick={handlePlus} className="w-4 h-4"/>
                  </button>
                </div>
        ...
        {relatedProducts.length > 0 && (
          <section className="mt-12 mb-44">
            <div className="flex items-center justify-between mb-6">
              ...
              <Link className="text-sm font-semibold text-app-orange hover:text-app-orange-dark flex items-center gap-1 transition-colors" to={`/products?category=${product.category}`} onClick={() => window.scrollTo(0,0)}>
              View all <ArrowRightIcon className="size-4" />
              </Link>
            </div>

            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 grid-cols-5 gap-4 xl:gap-8">
